...
/*! 
*  \brief  设置文本值 设置为字符串格式
*  \param  整数
*/
void SetTextValueInt32(uint16_t screen_id, uint16_t control_id,int_least16_t value)
{
	uint8_t buffer[12] = {0};
...
*  \param  一位小数
*/
void SetTextValueFloat1(uint16_t screen_id, uint16_t control_id,float value)
{
	uint8_t buffer[12] = {0};
	sprintf(buffer,"%.1lf",value);//把浮点数转换为字符串(保留1位小数)
	SetTextValue(screen_id,control_id,buffer);
}
/*! 
*  \brief  设置文本值 设置为字符串格式
*  \param  二位小数
*/
void SetTextValueFloat2(uint16_t screen_id, uint16_t control_id,float value)
{
	uint8_t buffer[12] = {0};
	sprintf(buffer,"%.2lf",value);//把浮点数转换为字符串(保留2位小数)
	SetTextValue(screen_id,control_id,buffer);
}
...
*  \param  strings 字符串内容
*/
void DisText(uint16_t x, uint16_t y,uint8_t back,uint8_t font,uchar *strings )
{
    BEGIN_CMD();
    TX_8(0x20);
    TX_16(x);
...
*  \param  value 按钮状态
*/
void SetButtonValue(uint16_t screen_id,uint16_t control_id,uchar state)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x10);
...
*  \param  str 文本值
*/
void SetTextValue(uint16_t screen_id,uint16_t control_id,uchar *str)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x10);
...
*  \brief      计算字符串长度
*/
uint32_t GetStringLen(uchar *str)
{
    uchar *p = str;
    while(*str)
    {
...
*  \param  strings 字符串
*/
void BatchSetText(uint16_t control_id,uchar *strings)
{
    TX_16(control_id);
